...
from core.settings_store import load_settings as _load_settings_json, save_settings as _save_settings_json
from core.users_store import load_users as _load_users_json, save_users as _save_users_json
from core.history_store import load_history as _load_history_json, append_history as _append_history_json
from core.subscriptions_store import (
    load_subscriptions as _load_push_subscriptions_json,
    save_subscriptions as _save_push_subscriptions_json,
)
...
try:
    ...
    try:
        from config import COMFY_DIR as _COMFY_DIR_CFG
    except ImportError:
        _COMFY_DIR_CFG = r"C:\AI\New_Comfy"
except ImportError:
    _COMFY_DIR_CFG       = r"C:\AI\New_Comfy"
...
# === KONFIGURACJA LOGOW ===
LOG_FILE = str(BASE_DIR / "serwer_comfy.log")
...
# Wycisz debugowy szum z bibliotek HTTP/push
for _noisy in ("httpx", "httpcore", "hpack", "h2", "h11", "urllib3", "requests"):
...
_CLEANUP_PREFIXES = ("telegram_result_", "mobile_", "tmp_", "temp_", "upload_")
_CLEANUP_EXTENSIONS = (".jpg", ".jpeg", ".png", ".webp", ".tmp")

def _cleanup_dir(directory: str, cutoff: float) -> int:
    """Usuwa stare pliki generowania/uploadu z danego katalogu. Zwraca liczbę usuniętych."""
    removed = 0
    try:
        if not os.path.isdir(directory):
            return 0
        for fname in os.listdir(directory):
            # Usuń tylko pliki pasujące do znanych prefiksów LUB rozszerzeń tymczasowych
            low = fname.lower()
            is_gen   = any(fname.startswith(p) for p in _CLEANUP_PREFIXES)
            is_tmp   = any(low.endswith(e) for e in (".tmp",))
            if not (is_gen or is_tmp):
                continue
...
def cleanup_old_files():
    """
    Czyści stare pliki (>7 dni) ze wszystkich katalogów:
    - globalny IMAGE_DIR
    - katalogi użytkowników z users.json
    - ComfyUI /input (pliki mobile_* i upload_*)
    """
    cutoff = time.time() - 7 * 24 * 3600
    total  = 0

    # 1. Globalny IMAGE_DIR
    total += _cleanup_dir(IMAGE_DIR, cutoff)

    # 2. Katalogi użytkowników (wczytujemy users.json jeśli istnieje)
    try:
        users_path = BASE_DIR / "users.json"
        if users_path.exists():
            with open(users_path, "r", encoding="utf-8") as _f:
                _users = json.load(_f)
            _seen_dirs = {IMAGE_DIR}
            for _uid, _udata in _users.items():
                _udir = _udata.get("image_dir", "")
                if _udir and _udir not in _seen_dirs and os.path.isdir(_udir):
                    _seen_dirs.add(_udir)
                    total += _cleanup_dir(_udir, cutoff)
    except Exception:
        pass

    # 3. ComfyUI /input — pliki z naszych uploaderów (mobile_*, upload_*)
    try:
        comfy_input = os.path.join(_COMFY_DIR_CFG, "ComfyUI", "input")
        if os.path.isdir(comfy_input):
            total += _cleanup_dir(comfy_input, cutoff)
    except Exception:
        pass

    if total:
        log.info(f"Auto-sprzatanie: usunieto {total} starych plikow (>7 dni)")
...
_startup_settings_path = BASE_DIR / "settings.json"
try:
    _s = _load_settings_json(_startup_settings_path)
    if _s.get("telegram_token"):
        TELEGRAM_BOT_TOKEN = _s["telegram_token"]
    if _s.get("telegram_chat_id"):
        TELEGRAM_CHAT_ID = _s["telegram_chat_id"]
    if _s.get("image_dir"):
        IMAGE_DIR = _s["image_dir"]
    if _s.get("comfy_url"):
        COMFY_URL = _s["comfy_url"]
    if _s.get("vram_free_wait"):
        VRAM_FREE_WAIT = _s["vram_free_wait"]
    if _s:
        log.info("Wczytano ustawienia z settings.json")
    _STARTUP_REMOTE_URL = _s.get("remote_url", "") if _s else ""
...
async def auth_middleware(request: Request, call_next):
    ...
    if path in public_paths or path.startswith("/static") or path.startswith("/login/otp"):
...
app.state.total_generated_count = 0   # łączna liczba wygenerowanych obrazów w tej sesji (dzienny)
app.state.queue_meta: dict      = {}   # prompt_id -> {uid, user_name, workflow_id, total_iter, queued_at}
app.state.gen_session_id: str   = ""   # UUID bieżącej sesji generowania (do grupowania w galerii)

_server_start_time = time.time()  # czas startu serwera
_comfy_ws_active = False  # True jeśli websocket-client zainstalowany i działa

# ── Dzienny licznik generacji (persist do pliku) ───────────────────────────
_GEN_COUNT_FILE = BASE_DIR / "gen_count_today.json"

def _load_today_count() -> int:
    """Wczytuje licznik dzienny z pliku. Resetuje jeśli data się zmieniła."""
    try:
        data = json.loads(_GEN_COUNT_FILE.read_text(encoding="utf-8"))
...
def _save_today_count(count: int):
    try:
        _GEN_COUNT_FILE.write_text(
            json.dumps({"date": time.strftime("%Y-%m-%d"), "count": count}),
            encoding="utf-8"
        )
    except Exception:
        pass
...
app.state.total_generated_count = _load_today_count()
...
def _auto_detect_tunnel_url():
    """
    Wykrywa URL tunelu (ngrok) przez lokalny API ngrok (port 4040).
    Odpala się w tle przy starcie serwera – jeśli URL nie jest jeszcze znany.
    Próbuje przez ~2 minuty, potem odpuszcza.
    """
    # Jeśli URL już ustawiony przez tray (statyczna domena), nie rób nic
    if app.state.remote_url:
        return

    deadline = time.time() + 120  # 2 minuty
    while time.time() < deadline:
        time.sleep(5)
        # Sprawdź najpierw app.state – może tray już ustawił
        if app.state.remote_url:
            return
        try:
            ...
            for t in tunnels:
                ...
                if pub_url.startswith("https://"):
                    app.state.remote_url = pub_url
                    log.info(f"_auto_detect_tunnel_url: znaleziono URL = {pub_url}")
                    try:
                        import qrcode
                        qr  = qrcode.make(pub_url)
                        buf = io.BytesIO()
                        qr.save(buf, format='PNG')
                        app.state.qr_base64_cache = base64.b64encode(buf.getvalue()).decode()
                    except Exception as qe:
                        log.debug(f"QR gen error: {qe}")
                    return
        except Exception:
            pass  # ngrok nie gotowy jeszcze – spróbuj za chwilę

    log.warning("_auto_detect_tunnel_url: nie wykryto URL tunelu w ciągu 2 minut")
...
async def on_startup():
    try:
        ...
        t.start()
    # Ustaw remote_url ze settings.json jeśli skonfigurowany
    if _STARTUP_REMOTE_URL and _STARTUP_REMOTE_URL.startswith("https://"):
        app.state.remote_url = _STARTUP_REMOTE_URL
        log.info(f"remote_url z settings.json: {_STARTUP_REMOTE_URL}")
    # Push przy starcie (w osobnym wątku żeby nie blokować startu)
    threading.Thread(target=_push_server_start, daemon=True).start()
    init_default_admin()
    _start_tg_watchdog()
    # Auto-wykryj URL tunelu tylko jeśli nie ustawiono w settings.json
    if not app.state.remote_url:
        threading.Thread(target=_auto_detect_tunnel_url, daemon=True).start()
...
def send_telegram_photo(out_path: str, iter_num: int, tg_token: str = "", tg_chat: str = "", caption: str = ""):
    ...
    _tg_data_base = {'chat_id': _TG_CHAT}
    if caption:
        _tg_data_base['caption'] = caption[:1024]  # Telegram limit
    ...
    for attempt in range(1, TELEGRAM_RETRY + 1):
        try:
            if file_size > TELEGRAM_MAX_BYTES:
                ...
                tg_resp = requests.post(url_photo, data=_tg_data_base,
    ...
    try:
        for i in range(iterations):
            ...
            for out_filename, out_subfolder, out_type in out_filenames:
                ...
                app.state.total_generated_count += 1
                _save_today_count(app.state.total_generated_count)

                if TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID:
                    send_telegram_photo(out_path, i + 1)
                # Push notification - tylko do właściciela generowania (z obrazkiem jako załącznik)
                try:
                    _uid = app.state.processing_uid
                    if _uid:
                        ...
                        send_push_to_all("✅ Generowanie gotowe",
                                        f"Iter {i+1}/{iterations} – {workflow_type}")
                except Exception:
                    pass
...
.queue-item-rich{background:#252528;border-radius:10px;padding:12px 13px;margin-bottom:8px}
.qi-header{display:flex;align-items:center;gap:10px;margin-bottom:8px}
.qi-avatar{width:32px;height:32px;border-radius:50%;background:#3a3a3e;display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:700;color:#bbb;flex-shrink:0}
.qi-meta{flex:1;min-width:0}
.qi-title{font-size:14px;font-weight:600;color:#e8e8e8;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.qi-user{font-size:11px;color:#666;margin-top:2px}
.q-progress-wrap{height:5px;background:#333;border-radius:3px;margin-bottom:4px;overflow:hidden}
.q-progress-bar{height:100%;background:#4CAF50;border-radius:3px;transition:width .4s}
.q-progress-bar.q-anim{background:#ff9800;animation:qanim 1.4s ease-in-out infinite}
@keyframes qanim{0%,100%{opacity:.5}50%{opacity:1}}
.q-progress-label{font-size:11px;color:#888;margin-bottom:5px}
.qi-chips{display:flex;flex-wrap:wrap;gap:5px}
.q-chip{font-size:11px;color:#888;background:#1e1e21;padding:2px 8px;border-radius:10px}
...
<div class="screen" id="screen-queue">
  <div class="card">
    ...
    <div id="q-running-list"><div class="queue-empty">Brak aktywnych zadań</div></div>
  </div>
  <div class="card">
    <div class="card-title">⏳ Oczekujące</div>
    <div id="q-pending-list"><div class="queue-empty">Kolejka pusta</div></div>
  </div>
  <button class="btn btn-red" onclick="clearQueue()" style="margin-top:4px">🛑 Przerwij i wyczyść kolejkę</button>
</div>
...
<div class="screen" id="screen-settings">
  ...
  <!-- Profil aktywny + przełącznik -->
  <div class="card">
    <div class="card-title">👤 Aktywny profil</div>
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px">
      <span id="profile-avatar" style="font-size:28px">👤</span>
      <div>
        <div id="profile-name-label" style="font-size:15px;font-weight:600;color:#eee">Brak profilu</div>
        <div style="font-size:11px;color:#555">Kliknij „Zmień" aby wybrać lub dodać profil</div>
      </div>
    </div>
    <button class="btn btn-dark btn-sm" onclick="openProfileSwitcher()" style="width:auto;padding:7px 16px">Zmień profil</button>
  </div>
  ...
  <div class="card">
    ...
    <div id="push-admin-tools" style="display:none;margin-top:12px;border-top:1px solid #1a1a1a;padding-top:10px">
      <div style="font-size:11px;color:#666;margin-bottom:6px">🔧 Narzędzia admina</div>
      <button class="btn btn-dark btn-sm" onclick="regenerateVapid()" style="background:#2a1000;border-color:#8b4000;color:#ffa040;font-size:11px">
        🔑 Regeneruj klucze VAPID
      </button>
      <div style="font-size:10px;color:#555;margin-top:4px">Wymaga ponownej subskrypcji na wszystkich urządzeniach</div>
      <div id="vapid-alert" style="display:none" class="alert-sm"></div>
    </div>
  ...
  <div class="card">
    ...
    <div style="margin-top:16px;border-top:1px solid #1a1a1a;padding-top:12px">
      <div class="card-title" style="margin-bottom:8px">🔒 Weryfikacja dwuetapowa (2FA)</div>
      <div style="font-size:12px;color:#666;margin-bottom:10px">Pierwsze logowanie z nowego urządzenia wymaga kodu OTP wysłanego przez Telegram. Kod ważny 10 minut.</div>
      <button class="btn btn-dark btn-sm" onclick="revokeDevices()" style="background:#3a1a00;border-color:#8b4000;color:#ffa040">
        🗑 Cofnij zaufanie wszystkim urządzeniom
      </button>
      <div id="revoke-alert" style="display:none" class="alert-sm"></div>
    </div>
  </div>
...
let _previewBlobUrl  = null;  // Blob URL zamiast inline base64 – iOS Safari nie GC'uje

function _applyPreviewImage(b64, imgEl) {
  // Konwertuj base64 → Blob URL; uniknij GC na iOS
  try {
    const bin = atob(b64);
...
async function _fetchPreview() {
  try {
    const r = await fetch('/api/preview?_t=' + Date.now());
    const d = await r.json();
    const box = document.getElementById('live-preview-box');
    const img = document.getElementById('live-preview-img');
    if (!box || !img) return;
    if (d.image) {
      _applyPreviewImage(d.image, img);
      box.style.display = 'block';
    } else if (!d.processing) {
      box.style.display = 'none';
    }
    // Jeśli processing=true ale brak obrazu — box zostaje (nie chowamy podczas generowania)
  } catch(e) {}
}

function startPreview() {
  if (_previewInterval) return;
  _fetchPreview();  // od razu pierwsze pobranie
  _previewInterval = setInterval(_fetchPreview, 2000);
}
...
// iOS: odśwież preview gdy użytkownik wróci do karty (tab focus)
document.addEventListener('visibilitychange', function() {
  if (!document.hidden && _previewInterval) {
...
async function loadGallery() {
  ...
  try {
    ...
    _galleryFiles = d.files;  // zapamiętaj do nawigacji w modalu

    // Grupuj po gen_session (obrazy z jednej generacji razem)
    var groups = [];
    var sessionMap = {};
    d.files.forEach(function(f) {
      var sid = f.gen_session || ('_solo_' + f.filename);
      if (!sessionMap[sid]) {
        sessionMap[sid] = [];
        groups.push({sid: sid, files: sessionMap[sid], ts: f.timestamp});
      }
      sessionMap[sid].push(f);
    });
    // Sortuj grupy od najnowszych
    groups.sort(function(a,b){ return b.ts - a.ts; });

    function _thumbCard(f, showActions) {
      var fJson = JSON.stringify(f).replace(/"/g, '&quot;');
      var label = f.style ? f.style.replace('/', '').trim() : '';
      var thumbUrl = '/gallery_thumb/' + f.filename + '?size=300';
      var h = '<div style="border-radius:10px;overflow:hidden;background:#111;position:relative">';
      h += '<img src="' + thumbUrl + '" loading="lazy" onclick="openGalleryModal(this)" data-f="' + fJson + '" style="width:100%;aspect-ratio:1;object-fit:cover;display:block;cursor:pointer" />';
      if (label) h += '<div style="position:absolute;bottom:0;left:0;right:0;background:linear-gradient(transparent,rgba(0,0,0,.8));padding:8px 6px 5px;font-size:10px;color:#ccc;line-height:1.3;pointer-events:none">' + label + '</div>';
      if (showActions) {
        h += '<div class="gallery-actions">';
        h += '<button class="gallery-btn" onclick="downloadGalleryImage(' + JSON.stringify(f.filename) + ', event)" title="Pobierz">⬇</button>';
        h += '<button class="gallery-btn" onclick="deleteGalleryImage(' + JSON.stringify(f.filename) + ', event)" title="Usun">🗑</button>';
        h += '</div>';
      }
      h += '</div>';
      return h;
    ...
    groups.forEach(function(g) {
      if (g.files.length === 1) {
        // Pojedynczy obraz – normalny kafelek
        html += _thumbCard(g.files[0], true);
      } else {
        // Grupa – pokaż pierwszy z badges, reszta ukryta, click expand
        var gid = 'grp_' + g.sid.replace(/[^a-z0-9]/gi,'_');
        var first = g.files[0];
        var fJson = JSON.stringify(first).replace(/"/g, '&quot;');
        var thumbUrl = '/gallery_thumb/' + first.filename + '?size=300';
        // Nagłówek grupy: miniatura + badge
        html += '<div style="border-radius:10px;overflow:hidden;background:#111;position:relative">';
        html += '<img src="' + thumbUrl + '" loading="lazy" onclick="toggleGalleryGroup(\\'' + gid + '\\', this)" data-f="' + fJson + '" style="width:100%;aspect-ratio:1;object-fit:cover;display:block;cursor:pointer" />';
        html += '<div style="position:absolute;top:4px;left:4px;background:rgba(0,0,0,.75);color:#4CAF50;font-size:11px;font-weight:700;padding:3px 8px;border-radius:10px;pointer-events:none">📦 ' + g.files.length + ' szt.</div>';
        html += '<div class="gallery-actions">';
        html += '<button class="gallery-btn" onclick="toggleGalleryGroup(\\'' + gid + '\\')" title="Rozwiń">⊞</button>';
        html += '</div></div>';
        // Rozwinięta siatka wewnętrzna (domyślnie ukryta)
        html += '<div id="' + gid + '" style="display:none;grid-column:1/-1;background:#111;border-radius:10px;padding:8px">';
        html += '<div style="font-size:12px;color:#555;margin-bottom:6px;padding:0 2px">' + g.files.length + ' grafiki z jednej generacji:</div>';
        html += '<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px">';
        g.files.forEach(function(fi) { html += _thumbCard(fi, true); });
        html += '</div></div>';
      }
    });
    grid.innerHTML = html || '<div style="color:#444;text-align:center;padding:30px;grid-column:1/-1">Brak wyników</div>';
  } catch(e) {
...
var _modalCurrentIndex = -1;
var _galleryFiles = [];  // kopia aktualnych plików galerii (do nawigacji)

function openGalleryModal(el) {
  var raw = el.getAttribute('data-f').replace(/&quot;/g, '"');
  const f = JSON.parse(raw);
  // Znajdź indeks w _galleryFiles
  _modalCurrentIndex = _galleryFiles.findIndex(function(gf){ return gf.filename === f.filename; });
  if (_modalCurrentIndex < 0) _modalCurrentIndex = 0;
  _showModalFile(f);
}

function _showModalFile(f) {
  _modalCurrentFile = f;
  ...
  if (f.prompt_full) {
    promptEl.textContent = f.prompt_full;
  ...
  if (ctr) ctr.textContent = (_modalCurrentIndex >= 0 && _galleryFiles.length > 0)
    ? (_modalCurrentIndex + 1) + ' / ' + _galleryFiles.length
    : '';
  // Przyciski akcji
  ...
  if (dlBtn) {
    ...
    dlBtn.onclick = function() { downloadGalleryImage(f.filename, {stopPropagation:function(){}}); };
  }
...
function galleryModalNav(dir) {
  if (!_galleryFiles.length) return;
  _modalCurrentIndex = (_modalCurrentIndex + dir + _galleryFiles.length) % _galleryFiles.length;
  _showModalFile(_galleryFiles[_modalCurrentIndex]);
}
...
function toggleGalleryGroup(gid, imgEl) {
  // Jeśli kliknięto w miniaturkę grupy: otwórz modal pierwszego lub rozwiń
  var grpDiv = document.getElementById(gid);
  if (!grpDiv) return;
  if (imgEl && grpDiv.style.display === 'none') {
    // Pierwsze kliknięcie – rozwiń grupę
    grpDiv.style.display = 'block';
  } else if (imgEl && grpDiv.style.display !== 'none') {
    // Drugie kliknięcie na miniaturę – otwórz modal
    openGalleryModal(imgEl);
  } else {
    grpDiv.style.display = grpDiv.style.display === 'none' ? 'block' : 'none';
  }
}

// Swipe support dla modala galerii
(function(){
  var _tx = 0;
  var el = document.getElementById('gallery-modal');
  if (!el) return;
  el.addEventListener('touchstart', function(e){ _tx = e.changedTouches[0].screenX; }, {passive:true});
  el.addEventListener('touchend', function(e){
    var dx = e.changedTouches[0].screenX - _tx;
...
async function initPush() {
  ...
  try {
    ...
    if (existing) {
      // Porównaj applicationServerKey z aktualnym kluczem serwera
      var keyMismatch = false;
      try {
        var akBuf = existing.options && existing.options.applicationServerKey;
        if (akBuf && akBuf.byteLength > 0) {
          var akB64 = btoa(String.fromCharCode.apply(null, new Uint8Array(akBuf)))
                        .replace(/\+/g,'-').replace(/\//g,'_').replace(/=+$/,'');
          var serverB64 = d.public_key.replace(/=+$/,'');
          keyMismatch = (akB64 !== serverB64);
          if (keyMismatch) console.warn('[push] Klucz VAPID niezgodny! Wymuszam re-subskrypcję. stored='+akB64.substring(0,20)+' server='+serverB64.substring(0,20));
        }
      } catch(keyErr) { console.warn('[push] Nie można odczytać applicationServerKey:', keyErr); }
      ...
      // Klucz niezgodny — wypisz starą subskrypcję i zarejestruj nową
      try {
        ...
        console.info('[push] Stara subskrypcja usunięta, tworzę nową...');
      } catch(unsubErr) { console.warn('[push] Błąd przy wypisywaniu:', unsubErr); }
      // iOS wymaga gestu użytkownika do subscribe — pokaż przycisk
      status.textContent = '⚠️ Wymagana ponowna subskrypcja (klucz zmieniony)';
      status.style.color = '#f90';
      if (btn) { btn.textContent = 'Włącz powiadomienia'; btn.style.background = ''; }
      return;
    }
...
function simpleStartLivePreview() {
  ...
  async function _simpleFetch() {
    try {
      var r = await fetch('/api/preview?_t=' + Date.now());
      var d = await r.json();
      ...
      if (d.image) {
        ...
        // Generowanie trwa ale brak obrazu jeszcze — pokaż placeholder
        if (wrap && !img.src) wrap.style.display = 'block';
      }
    } catch(e) {}
  }
  _simpleFetch();
  _previewInterval = setInterval(_simpleFetch, 2000);
}
...
async function loadProfiles() {
  ...
  try {
    ...
    var pushAdminTools = document.getElementById('push-admin-tools');
    if (pushAdminTools && dm.role === 'admin') pushAdminTools.style.display = '';
...
function renderQueue(cid, items, type, empty) {
  ...
  el.innerHTML = items.map(item => {
    const wfName   = item.workflow_name || item.workflow_id || '—';
    const userName = item.user_name || '?';
    const avatar   = userName.charAt(0).toUpperCase();
    const badge    = type === 'running'
      ? '<span class="q-badge running">⚡ Aktywne</span>'
      : '<span class="q-badge pending">#' + item.position + '</span>';

    // Pasek postępu kroków
    let stepHtml = '';
    if (type === 'running') {
      const sm = item.step_max || 0, sv = item.step_value || 0;
      if (sm > 0) {
        const pct = Math.round(sv / sm * 100);
        stepHtml = '<div class="q-progress-wrap"><div class="q-progress-bar" style="width:' + pct + '%"></div></div>'
                 + '<div class="q-progress-label">Krok: ' + sv + ' / ' + sm + ' (' + pct + '%)</div>';
      } else {
        stepHtml = '<div class="q-progress-wrap"><div class="q-progress-bar q-anim" style="width:45%"></div></div>'
                 + '<div class="q-progress-label">' + (item.current_node || 'Przetwarzanie…') + '</div>';
      }
    }

    // Chip: iteracje
    let iterChip = '';
    const totalIter = item.total_iter || 0;
    if (totalIter > 0) {
      const curIter = type === 'running' ? (item.current_iter || 1) : 0;
      iterChip = '<span class="q-chip">🔁 ' + curIter + ' / ' + totalIter + ' generowań</span>';
    }

    // Chip: czas w kolejce
    let timeChip = '';
    if (item.queued_at) {
      const secs = Math.floor(Date.now() / 1000 - item.queued_at);
      const timeStr = secs < 60 ? secs + 's' : Math.floor(secs / 60) + 'min';
      timeChip = '<span class="q-chip">⏱ ' + timeStr + ' temu</span>';
    }

    return '<div class="queue-item-rich">'
      + '<div class="qi-header">'
      + '<div class="qi-avatar">' + avatar + '</div>'
      + '<div class="qi-meta"><div class="qi-title">' + wfName + '</div><div class="qi-user">' + userName + '</div></div>'
      + badge
      + '</div>'
      + stepHtml
      + (iterChip || timeChip ? '<div class="qi-chips">' + iterChip + timeChip + '</div>' : '')
      + '</div>';
  }).join('');
}
...
async function revokeDevices() {
  if (!confirm('Cofnąć zaufanie WSZYSTKIM zapamiętanym urządzeniom? Przy następnym logowaniu będzie wymagany kod OTP.')) return;
  try {
    const r = await fetch('/api/2fa/revoke_devices', {method:'POST',
      headers:{'Content-Type':'application/json'}, body:JSON.stringify({})});
    const d = await r.json();
    if (d.status === 'ok') showAlert('revoke-alert', `Usunięto ${d.removed} zaufanych urządzeń.`, true);
    else showAlert('revoke-alert', d.message || 'Błąd', false);
  } catch(e) { showAlert('revoke-alert', 'Błąd połączenia', false); }
}

async function regenerateVapid() {
  if (!confirm('Zregenerować klucze VAPID Push?\\n\\nWszystkie subskrypcje zostaną usunięte.\\nKażde urządzenie będzie musiało ponownie włączyć powiadomienia.')) return;
  try {
    const r = await fetch('/api/push/regenerate-vapid', {method:'POST', headers:{'Content-Type':'application/json'}, body:'{}'});
    const d = await r.json();
    if (d.status === 'ok') {
      showAlert('vapid-alert', 'Klucze VAPID zregenerowane. Wyłącz i włącz powiadomienia ponownie.', 'ok');
      // Wypisz obecną subskrypcję przeglądarki
      if (_pushSubscription) { await _pushSubscription.unsubscribe().catch(()=>{}); _pushSubscription = null; }
      document.getElementById('push-status').textContent = 'Powiadomienia wyłączone – wymagana ponowna subskrypcja';
      document.getElementById('push-status').style.color = '#888';
      var pb = document.getElementById('push-btn');
      if (pb) { pb.textContent = 'Włącz powiadomienia'; pb.style.background = ''; }
    } else {
      showAlert('vapid-alert', d.message || 'Błąd', 'err');
    }
  } catch(e) { showAlert('vapid-alert', 'Błąd połączenia: ' + e.message, false); }
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function tryLoad() {
    ...
    try {
      ...
      // Zaladuj workflow niezaleznie od stanu ComfyUI
      await loadWorkflows();
      loadPresets();
      ...
      if (attempts < 5) setTimeout(tryLoad, 2000);
      else {
...
<div id="gallery-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:1000;overflow-y:auto;padding:16px">
  <div style="max-width:500px;margin:0 auto">
    ...
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px">
      <button onclick="galleryModalNav(-1)" style="background:#222;border:1px solid #333;color:#aaa;border-radius:8px;padding:8px 16px;font-size:18px;flex:0 0 auto">‹</button>
      <span id="modal-counter" style="flex:1;text-align:center;font-size:12px;color:#555">—</span>
      <button onclick="galleryModalNav(+1)" style="background:#222;border:1px solid #333;color:#aaa;border-radius:8px;padding:8px 16px;font-size:18px;flex:0 0 auto">›</button>
    </div>
    ...
    <div class="card">
      ...
      <div id="modal-suffix-row" class="detail-row"><span class="detail-key">Suffix</span><span class="detail-val" id="modal-suffix" style="word-break:break-word;white-space:pre-wrap">—</span></div>
      <div id="modal-prompt-row" class="detail-row" style="display:none"><span class="detail-key">Prompt CLIP</span><span class="detail-val" id="modal-prompt" style="word-break:break-word;white-space:pre-wrap;font-size:11px;color:#aaa;max-height:100px;overflow-y:auto;display:block">—</span></div>
...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        body { font-family:'Segoe UI',sans-serif; background:#0a0a0d; color:#e0e0e0; padding:24px; margin:0; }
        .grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(300px,1fr)); gap:16px; max-width:1200px; margin:auto; }
        .grid-wide { display:grid; grid-template-columns:1fr 1fr; gap:16px; max-width:1200px; margin:16px auto 0; }
        @media(max-width:700px){ .grid-wide { grid-template-columns:1fr; } }
        .card { background:#1a1a22; border:1px solid #2a2a38; border-radius:14px; padding:18px; }
        h2 { color:#fff; margin:0 0 12px 0; font-size:1rem; border-bottom:1px solid #2a2a38; padding-bottom:10px; display:flex; align-items:center; gap:8px; }
        .metric { display:flex; justify-content:space-between; align-items:center; margin:10px 0; font-size:1rem; }
        .val { font-weight:bold; color:#4CAF50; }
        .val.warn { color:#ff9800; } .val.danger { color:#f44336; } .val.blue { color:#2196F3; }
        .btn { width:100%; padding:11px; background:#f44336; color:white; border:none; border-radius:8px; font-size:14px; font-weight:bold; cursor:pointer; margin-top:8px; transition:opacity .2s; }
        .btn:hover { opacity:.85; } .btn.blue { background:#2196F3; } .btn.green { background:#388e3c; }
        .header-title { text-align:center; margin-bottom:24px; }
        .header-title h1 { margin:0 0 10px; font-size:1.5rem; color:#fff; }
        .badge { display:inline-block; padding:3px 9px; border-radius:20px; font-size:11px; font-weight:700; margin-left:6px; }
        .badge-green { background:#1b5e20; color:#69f0ae; }
        .badge-yellow { background:#4a3500; color:#ffd740; }
        .badge-red { background:#4e0000; color:#ff5252; }
        /* progress bar generowania */
        .gen-bar-wrap { background:#111; border-radius:6px; height:8px; margin:8px 0; overflow:hidden; }
        .gen-bar { height:8px; background:linear-gradient(90deg,#4CAF50,#81c784); border-radius:6px; transition:width .5s; }
        /* kolejka */
        .queue-dot { width:10px;height:10px;border-radius:50%;display:inline-block;margin-right:6px; }
        /* log */
        #log-box { background:#0d0d12; border:1px solid #222; border-radius:8px; padding:10px; font-family:'Consolas',monospace; font-size:11px; color:#888; height:160px; overflow-y:auto; white-space:pre-wrap; word-break:break-all; }
        /* galeria */
        .thumb-grid { display:flex; flex-wrap:wrap; gap:6px; margin-top:8px; }
        .thumb-grid img { width:80px; height:80px; object-fit:cover; border-radius:6px; border:1px solid #2a2a38; cursor:pointer; transition:transform .15s; }
        .thumb-grid img:hover { transform:scale(1.08); border-color:#4CAF50; }
        /* sesje */
        .session-row { display:flex; justify-content:space-between; align-items:center; padding:6px 0; border-bottom:1px solid #222; font-size:13px; }
        .session-row:last-child { border-bottom:none; }
        /* uptime */
        #uptime-val { font-size:1.3rem; font-weight:700; color:#4CAF50; }
    </style>
</head>
<body>
    <div class="header-title">
        <h1>⚙️ Silnik ComfyUI — Panel Zarządzania</h1>
        <div style="display:flex;gap:10px;justify-content:center;flex-wrap:wrap">
            <a href="/kreator" target="_blank" style="padding:7px 16px;background:#1b5e20;color:#69f0ae;border-radius:8px;font-size:13px;font-weight:700;text-decoration:none">⚙️ Kreator workflow</a>
            <a href="/" target="_blank" style="padding:7px 16px;background:#0d47a1;color:#82b1ff;border-radius:8px;font-size:13px;font-weight:700;text-decoration:none">📱 Interfejs mobilny</a>
            <a href="/admin" target="_blank" style="padding:7px 16px;background:#311b92;color:#b388ff;border-radius:8px;font-size:13px;font-weight:700;text-decoration:none">👤 Admin</a>
        </div>
    </div>

    <!-- Rząd 1 — pierwotne karty -->
    <div class="grid">
        <!-- GPU -->
        <div class="card">
            <h2>🎮 Monitor GPU</h2>
            <div class="metric"><span>Temperatura:</span><span id="gpu-temp" class="val">...</span></div>
            <div class="metric"><span>Użycie Rdzenia:</span><span id="gpu-util" class="val">...</span></div>
            <div class="metric"><span>Pamięć VRAM:</span><span id="gpu-vram" class="val">...</span></div>
        </div>

        <!-- Status + generowanie -->
        <div class="card">
            <h2>🖥️ Status Silnika</h2>
            <div class="metric"><span>Stan ComfyUI:</span><span id="comfy-realtime" class="val blue">...</span></div>
            <div class="metric"><span>Ostatni Workflow:</span><span id="last-wf" class="val" style="color:#aaa">Brak</span></div>
            <div class="metric"><span>Uptime:</span><span id="uptime-val">—</span></div>
            <div style="margin-top:12px;color:#888;font-size:0.85rem">Akcja:</div>
            <div id="current-action" style="color:#4CAF50;font-weight:bold;font-size:1rem;margin-top:4px">Czekam...</div>
            <div id="gen-progress-wrap" style="display:none;margin-top:8px">
                <div style="font-size:11px;color:#666;margin-bottom:3px">
                    Iter <span id="gen-cur">0</span>/<span id="gen-total">0</span>
                </div>
                <div class="gen-bar-wrap"><div class="gen-bar" id="gen-bar" style="width:0%"></div></div>
            </div>
            <div class="metric" style="margin-top:12px"><span>Wygenerowano dziś:</span><span id="gen-count" class="val">0</span></div>
        </div>

        <!-- Dostęp Zdalny -->
        <div class="card">
            <h2>🌐 Dostęp Zdalny</h2>
            <div id="qr-status" style="color:#888;font-size:0.9rem;margin-bottom:8px">Oczekuje na tunel...</div>
            <div id="qr-url" style="color:#4CAF50;font-size:0.8rem;word-break:break-all;margin-bottom:10px"></div>
            <div style="text-align:center"><img id="qr-img" src="" style="display:none;width:160px;height:160px;border-radius:8px;background:#fff;padding:6px" /></div>
            <button id="copy-url-btn" class="btn blue" onclick="copyUrl()" style="display:none;margin-top:10px">📋 Kopiuj URL</button>
        </div>

        <!-- Kolejka ComfyUI -->
        <div class="card">
            <h2>📋 Kolejka ComfyUI</h2>
            <div class="metric">
                <span><span class="queue-dot" style="background:#4CAF50"></span>Przetwarzane:</span>
                <span id="q-running" class="val">—</span>
            </div>
            <div class="metric">
                <span><span class="queue-dot" style="background:#ff9800"></span>Oczekujące:</span>
                <span id="q-pending" class="val" style="color:#ff9800">—</span>
            </div>
            <div id="q-status" style="font-size:11px;color:#555;margin-top:6px">Odpytywanie ComfyUI...</div>
            <button class="btn green" onclick="freeVram()" style="margin-top:12px">🧹 Wyczyść VRAM</button>
        </div>
    </div>

    <!-- Rząd 2 — log + sesje + galeria + kontrola -->
    <div class="grid-wide">
        <!-- Live Log -->
        <div class="card">
            <h2>📜 Live Log <span id="log-badge" class="badge badge-green">OK</span></h2>
            <div id="log-box">Ładowanie logu...</div>
            <div style="display:flex;gap:8px;margin-top:8px;flex-wrap:wrap;align-items:center">
                <label style="font-size:12px;color:#555;display:flex;align-items:center;gap:4px">
                    <input type="checkbox" id="log-autoscroll" checked> Auto-scroll
                </label>
                <label style="font-size:12px;color:#555">Filtr:</label>
                <select id="log-filter" style="background:#1a1a1a;color:#ccc;border:1px solid #333;border-radius:6px;font-size:12px;padding:2px 6px">
                    <option value="smart" selected>Inteligentny (bez szumu)</option>
        ...
        <!-- Aktywne Sesje + Kontrola Awaryjna -->
        <div style="display:flex;flex-direction:column;gap:16px">
            <div class="card">
                <h2>👤 Aktywne Sesje <span id="sess-badge" class="badge badge-green">0</span></h2>
                <div id="sessions-list" style="color:#555;font-size:13px">Ładowanie...</div>
            </div>
            <div class="card" style="border-color:#b71c1c">
                <h2 style="color:#ef9a9a">⚠️ Kontrola Awaryjna</h2>
                <p style="font-size:0.85rem;color:#666;margin:0 0 10px">Całkowite wyłączenie silnika ComfyUI i serwera.</p>
                <button class="btn" onclick="shutdownEngine()">🔴 WYŁĄCZ SILNIK</button>
            </div>
        </div>
    </div>

    <!-- Rząd 3 — Ostatnie obrazy -->
    <div style="max-width:1200px;margin:16px auto 0">
        <div class="card">
            <h2>🖼️ Ostatnio wygenerowane obrazy</h2>
            <div id="thumb-gallery" class="thumb-grid">
                <span style="color:#555;font-size:13px">Ładowanie...</span>
            </div>
        </div>
    </div>
    ...
    document.getElementById('log-autoscroll').addEventListener('change', e => { logAutoScroll = e.target.checked; });
    document.getElementById('log-filter').addEventListener('change', () => refreshLog());
    ...
    // ── Status laptopa (GPU, ComfyUI, generowanie) — co 2s ──────────────
    setInterval(async () => {
        try {
            const d = await fetch('/api/laptop_status').then(r => r.json());
        ...
        } catch(e) { document.getElementById('current-action').innerText = 'Błąd połączenia!'; }
    }, 2000);

    // ── Uptime + progress generowania — co 3s ────────────────────────────
    setInterval(async () => {
        try {
            const d = await fetch('/api/panel/uptime').then(r => r.json());
            document.getElementById('uptime-val').textContent = fmtUptime(d.uptime_s);
            document.getElementById('gen-count').textContent  = d.total_generated;
            const wrap = document.getElementById('gen-progress-wrap');
            if (d.is_processing && d.total_iter > 0) {
                wrap.style.display = 'block';
                document.getElementById('gen-cur').textContent   = d.current_iter;
                document.getElementById('gen-total').textContent = d.total_iter;
                document.getElementById('gen-bar').style.width   = Math.round(d.current_iter / d.total_iter * 100) + '%';
            } else {
    ...
    // ── Kolejka ComfyUI — co 5s ───────────────────────────────────────────
    setInterval(async () => {
        try {
            const d = await fetch('/api/panel/queue').then(r => r.json());
            if (!d.available) {
                document.getElementById('q-running').textContent = '—';
                document.getElementById('q-pending').textContent = '—';
                document.getElementById('q-status').textContent  = 'ComfyUI niedostępny';
                return;
            }
            const ri = d.running_images || d.running || 0;
            const pi = d.pending_images || d.pending || 0;
            document.getElementById('q-running').textContent = ri + (ri !== d.running ? ' grafik' : '');
            document.getElementById('q-pending').textContent = pi + (pi !== d.pending ? ' grafik' : '');
            document.getElementById('q-running').className   = 'val';
            document.getElementById('q-pending').className   = d.pending > 0 ? 'val warn' : 'val';
            document.getElementById('q-status').textContent  = d.running || d.pending ? 'ComfyUI pracuje' : 'Kolejka pusta';
        } catch(e) {}
    }, 5000);

    // ── QR / URL tunelu — polling ─────────────────────────────────────────
    ...
    function copyUrl() {
        ...
        setTimeout(() => { btn.textContent = '📋 Kopiuj URL'; }, 2000);
    }
    ...
    async function refreshLog() {
        try {
            ...
            const d = await fetch(`/api/panel/log_tail?n=80&filter=${filterVal}`).then(r => r.json());
            const lines = d.lines || [];
            const box = document.getElementById('log-box');
            box.textContent = lines.join('\\n');
            if (logAutoScroll) box.scrollTop = box.scrollHeight;
            const errCount = lines.filter(l => /\\bERROR\\b|\\bCRITICAL\\b|Exception/i.test(l)).length;
            const badge = document.getElementById('log-badge');
            badge.className = errCount > 0 ? 'badge badge-red' : 'badge badge-green';
            badge.textContent = errCount > 0 ? `${errCount} błąd` : 'OK';
        } catch(e) {}
    }
    refreshLog();
    setInterval(refreshLog, 4000);

    // ── Aktywne Sesje — co 10s ────────────────────────────────────────────
    async function refreshSessions() {
        try {
            const d = await fetch('/api/panel/active_sessions').then(r => r.json());
            const badge = document.getElementById('sess-badge');
            if (d.error) {
                document.getElementById('sessions-list').textContent = d.error;
                badge.textContent = '?'; badge.className = 'badge badge-yellow';
                return;
            }
            badge.textContent = d.count;
            badge.className = d.count > 0 ? 'badge badge-green' : 'badge badge-yellow';
            if (!d.sessions.length) {
                document.getElementById('sessions-list').innerHTML = '<span style="color:#444">Brak aktywnych sesji</span>';
                return;
            }
            document.getElementById('sessions-list').innerHTML = d.sessions.map(s =>
                '<div class="session-row">' +
                '<span>' + (s.name || s.uid) + '</span>' +
                '<span style="color:#555;font-size:11px">wygasa za ' + s.expires_in_min + ' min</span>' +
                '</div>'
            ).join('');
        } catch(e) {}
    }
    ...
    // ── Galeria miniatur — co 15s ─────────────────────────────────────────
    async function refreshGallery() {
        try {
            const d = await fetch('/api/panel/recent_images?limit=8').then(r => r.json());
            const gallery = document.getElementById('thumb-gallery');
            if (!d.images || !d.images.length) {
                gallery.innerHTML = '<span style="color:#444;font-size:13px">Brak wygenerowanych obrazów</span>';
                return;
            }
            gallery.innerHTML = d.images.map(img =>
                '<img src="data:image/jpeg;base64,' + img.thumb + '" title="' + img.filename + ' (' + img.size_kb + ' KB)" />'
            ).join('');
        } catch(e) {}
    }
    refreshGallery();
    ...
    async function freeVram() {
        try {
            const r = await fetch('/api/free_vram', {method:'POST'});
            const d = await r.json();
            const btn = document.querySelector('.btn.green');
            btn.textContent = d.status === 'ok' ? '✅ VRAM wyczyszczony' : '❌ Błąd';
            setTimeout(() => { btn.textContent = '🧹 Wyczyść VRAM'; }, 3000);
        } catch(e) {}
    }

    // ── Shutdown ──────────────────────────────────────────────────────────
    async function shutdownEngine() {
        if (!confirm('Czy na pewno chcesz wyłączyć silnik ComfyUI i serwer?')) return;
        try {
            await fetch('/shutdown_engine', {method:'POST'});
            document.body.innerHTML = "<div style='text-align:center;color:#f44336;margin-top:80px;font-size:2rem'>Serwer wyłączony.</div>";
        } catch(e) {}
    }
    </script>
</body>
...
async def get_queue_details():
    ...
    def _build_queue_item(item, position: int, is_running: bool) -> dict:
        pid  = item[1] if len(item) > 1 else "?"
        meta = app.state.queue_meta.get(pid, {})
        wf_id   = meta.get("workflow_id") or app.state.last_workflow or "?"
        # Próbuj pobrać przyjazną nazwę workflow z konfiguracji
        wf_name = wf_id
        try:
            cfg = wm.load_configs().get(wf_id, {})
            wf_name = cfg.get("name", wf_id) or wf_id
        except Exception:
            pass
        d = {
            "prompt_id":    pid,
            ...
            "user_name":    meta.get("user_name", app.state.processing_uid or "?"),
            "total_iter":   meta.get("total_iter", app.state.total_iter or 0),
            "queued_at":    meta.get("queued_at", 0),
            "position":     position,
        }
        if is_running:
            d["current_iter"] = app.state.current_iter
            d["step_value"]   = app.state.step_value
            d["step_max"]     = app.state.step_max
            d["current_node"] = app.state.current_node
        return d
    ...
    if q:
        ...
        result["queue_running_details"] = [
            _build_queue_item(item, i, True)  for i, item in enumerate(running)
        ]
        result["queue_pending_details"] = [
            _build_queue_item(item, i + 1, False) for i, item in enumerate(pending)
        ]
    try:
        gpu = get_gpu_info()
...
def _extract_positive_prompt(workflow: dict) -> str:
    """Fallback: czyta wejście 'text' CLIPTextEncode z template.
    Jeśli text jest linkiem – czyta TYLKO bezpośrednie string-i ze źródłowego noda,
    BEZ dalszej rekurencji. Pełny tekst pochodzi z _extract_prompt_from_history."""
    try:
        _SKIP_FIELDS = {
            "seed", "steps", "cfg", "sampler_name", "scheduler", "denoise",
            "model", "clip", "vae", "latent_image", "width", "height",
            "batch_size", "control_net", "image", "mask", "upscale_model",
            "weight_dtype", "noise", "guider", "sampler", "sigmas",
            "positive", "negative", "conditioning", "pixels", "samples",
            "upscale_method", "crop", "interpolation", "sharpness",
            "filename_prefix", "format", "quality", "lora_name",
            "strength_model", "strength_clip", "ckpt_name", "vae_name",
            "clip_name", "unet_name", "control_net_name",
        }
        _SEP_FIELDS = ("separator", "delimiter", "join_with", "joiner", "splitter")

        def _direct_strings(nid: str, follow_links: bool = True) -> str:
            """Zbiera string-i ze źródłowego noda.
            follow_links=True: idzie o JEDEN poziom w linki (string_b itp.),
            ale nie dalej – zbiera tylko ich bezpośrednie stringi."""
            node = workflow.get(str(nid), {})
            if not isinstance(node, dict):
                return ""
            inputs = node.get("inputs", {})
            sep = ""
            for sf in _SEP_FIELDS:
                if sf in inputs and isinstance(inputs[sf], str):
                    sep = inputs[sf]
                    break
            parts = []
            for key in sorted(inputs.keys()):
                if key in _SKIP_FIELDS or key in _SEP_FIELDS:
                    continue
                val = inputs[key]
                if isinstance(val, str) and val.strip():
                    parts.append(val.strip())
                elif follow_links and isinstance(val, list) and len(val) >= 1 and isinstance(val[0], (str, int)):
                    # Jeden poziom głębiej – tylko bezpośrednie stringi z linkowanego noda
                    sub = _direct_strings(str(val[0]), follow_links=False)
                    if sub:
                        parts.append(sub)
            return sep.join(parts) if parts else ""

        def _read_clip_text(clip_nid: str) -> str:
            node = workflow.get(str(clip_nid), {})
            if not isinstance(node, dict):
                return ""
            text_val = node.get("inputs", {}).get("text", "")
            if isinstance(text_val, str):
                return text_val.strip()
            if isinstance(text_val, list) and len(text_val) >= 1 and isinstance(text_val[0], (str, int)):
                result = _direct_strings(str(text_val[0]))
                log.debug(f"[prompt] CLIPTextEncode nid={clip_nid} ←nod[{text_val[0]}] direct: {result[:80]}")
                return result
            return ""
        ...
        for nid, node in workflow.items():
            ...
            if "CLIPTextEncode" in node.get("class_type", ""):
                text = _read_clip_text(nid)
                if len(text) > len(best):
                    best = text
        if best:
            log.debug(f"[prompt] template fallback: {best[:80]}")
            return best[:2000]

    except Exception as e:
        log.debug(f"[prompt] wyjatek: {e}")
    log.debug("[prompt] nie znaleziono promptu")
    return ""


def _read_prompt_from_txt(since_time: float) -> str:
    """Szuka nowych .txt w ComfyUI output po generacji (overwrite mode).
    Zwraca całą zawartość pliku – prompt może mieć wiele wierszy."""
    try:
        output_dir = os.path.join(_COMFY_DIR_CFG, "ComfyUI", "output")
        if not os.path.isdir(output_dir):
            log.debug(f"[prompt_txt] brak katalogu: {output_dir}")
            return ""
        candidates = []
        ...
        if not candidates:
            log.debug(f"[prompt_txt] brak nowych .txt od {since_time:.1f} w {output_dir}")
            return ""
        ...
        if content:
            log.info(f"[prompt_txt] {os.path.basename(newest)} ({len(content)} zn.): {content[:80]}")
            return content
    except Exception as e:
        log.debug(f"[prompt_txt] wyjatek: {e}")
    return ""


def _extract_prompt_from_history(workflow: dict, history_entry: dict) -> str:
    """Wyciąga faktyczny prompt z historii ComfyUI po wykonaniu.

    Strategia (bez hardkodowania numerów nodów, działa z każdym workflow):
    1. Zbiera source_nid dla każdego CLIPTextEncode (nod z którego czyta text).
    2. Szuka w historii (outputs) noda który:
       - czyta text z tego samego source_nid (SaveText, ShowText, PreviewTextNode itp.)
       - LUB jest samym source_nidem (jeśli trafił do outputs)
    3. Zwraca znaleziony tekst – bez żadnego traversalu grafu.
    """
    try:
        outputs = history_entry.get("outputs", {})
        best = ""

        def _first_string(node_out: dict) -> str:
        ...
        # Krok 1: zbierz source_nid-y CLIPTextEncode
        clip_sources: set = set()
        for nid, node in workflow.items():
            if not isinstance(node, dict):
                continue
            ...
            if isinstance(text_val, str) and text_val.strip():
                candidate = text_val.strip()
                if len(candidate) > len(best):
                    best = candidate
                log.debug(f"[prompt_hist] CLIP nid={nid} text bezposredni: {candidate[:80]}")
            elif isinstance(text_val, list) and len(text_val) >= 1:
                clip_sources.add(str(text_val[0]))

        if best:
            return best  # CLIPTextEncode ma tekst wprost

        # Krok 2: szukaj w workflow noda który czyta z clip_source
        # i jest w historii outputs (SaveText, ShowText, PreviewTextNode itp.)
        for nid, node in workflow.items():
            if not isinstance(node, dict):
                continue
            inputs = node.get("inputs", {})
            # Sprawdź każde wejście tekstowe tego noda
            for field in ("text", "string", "input"):
                fval = inputs.get(field)
                if isinstance(fval, list) and len(fval) >= 1 and str(fval[0]) in clip_sources:
                    node_out = outputs.get(str(nid), {})
                    if node_out:
                        candidate = _first_string(node_out)
                        if candidate and len(candidate) > len(best):
                            best = candidate
                            log.info(f"[prompt_hist] via output-node nid={nid} "
                                     f"ct={node.get('class_type','')} "
                                     f"← source={fval[0]}: {candidate[:80]}")

        # Krok 3: sprawdź czy sam source_nid jest w outputs
        if not best:
            for src in clip_sources:
                node_out = outputs.get(src, {})
                if node_out:
                    candidate = _first_string(node_out)
                    if candidate and len(candidate) > len(best):
                        best = candidate
                        log.info(f"[prompt_hist] source nid={src} w outputs: {candidate[:80]}")

        if not best:
            log.debug(f"[prompt_hist] brak w outputs; clip_sources={clip_sources}; "
                      f"outputs keys={list(outputs.keys())[:20]}")
        return best

    except Exception as e:
        log.debug(f"[prompt_hist] wyjatek: {e}")
    return ""
...
async def get_gallery(request: Request):
    ...
    try:
        files = []
        meta = load_gallery_meta(img_dir=_img_dir)
        for fname in os.listdir(_img_dir):
            ...
            files.append({
                ...
                "gen_session": fmeta.get("gen_session", ""),
            })
...
async def login_page(request: Request):
    ...
    # Walidacja next: tylko ścieżki lokalne (zapobiega open redirect)
    from urllib.parse import unquote
    next_url = unquote(next_url)
    if not next_url.startswith("/") or next_url.startswith("//"):
        next_url = "/"
    error_msg   = "Nieprawidłowy login lub hasło." if error else ""
    hidden_next = f'<input type="hidden" name="next" value="{next_url}">' if next_url and next_url != "/" else ""
...
<div class="box">
  <div class="logo">🎨</div>
  <h2>ComfyUI Mobile</h2>""" + (f'<div class="err">{error_msg}</div>' if error_msg else "") + f"""
...
async def login_submit(request: Request, username: str = Form(...), password: str = Form(...),
    ...
    # Walidacja next (open redirect guard)
    from urllib.parse import unquote
    next_url = unquote(next or "/")
    if not next_url.startswith("/") or next_url.startswith("//"):
        next_url = "/"
    users = load_users()
    ...
    if matched_user:
        # 2FA: sprawdź czy to zaufane urządzenie
        if _is_trusted_device(request, matched_uid):
            # Zaufane urządzenie — loguj od razu
            token = create_session(matched_uid)
            resp = RedirectResponse(url=next_url, status_code=302)
            resp.set_cookie("session_token", token, max_age=SESSION_DURATION,
                            httponly=True, samesite="lax")
            log.info(f"Zalogowano (trusted device): {matched_uid} → {next_url}")
            return resp
        # Nowe urządzenie — wyślij OTP i przekieruj na stronę weryfikacji
        otp_code = _generate_otp(matched_uid)
        threading.Thread(target=_send_otp_telegram, args=(matched_uid, otp_code), daemon=True).start()
        # Zakoduj kontekst w podpisanym tokenie tymczasowym
        tmp_token = secrets.token_hex(16)
        _active_sessions[f"otp_{tmp_token}"] = {
            "uid": matched_uid,
            "next": next_url,
            "expires": time.time() + _OTP_TTL
        }
        log.info(f"2FA: wysłano OTP dla {matched_uid}, nowe urządzenie")
        return RedirectResponse(url=f"/login/otp?t={tmp_token}", status_code=302)
    return RedirectResponse(url="/login?error=1", status_code=302)


@app.get("/login/otp")
async def otp_page(request: Request, t: str = ""):
    """Strona weryfikacji OTP (2FA)."""
    entry = _active_sessions.get(f"otp_{t}")
    if not entry or time.time() > entry.get("expires", 0):
        return RedirectResponse(url="/login?error=otp_expired", status_code=302)
    u = get_user_by_id(entry["uid"])
    tg_info = ""
    tg_token = u.get("telegram_token", "") or TELEGRAM_BOT_TOKEN
    tg_chat  = u.get("telegram_chat", "") or TELEGRAM_CHAT_ID
    if tg_token and tg_chat:
        tg_info = "<p style='color:#666;font-size:13px;text-align:center;margin-bottom:16px'>Kod wysłany przez Telegram 📱</p>"
    else:
        tg_info = "<p style='color:#f44;font-size:12px;text-align:center;margin-bottom:16px'>⚠️ Brak Telegrama — sprawdź logi serwera</p>"
    html = f"""<!DOCTYPE html><html lang="pl"><head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Weryfikacja 2FA – ComfyUI</title>
<style>
*{{box-sizing:border-box;margin:0;padding:0}}
body{{background:#0a0a0a;color:#eee;font-family:-apple-system,sans-serif;
     display:flex;align-items:center;justify-content:center;min-height:100vh}}
.box{{background:#141414;border:1px solid #222;border-radius:16px;padding:36px 28px;width:90%;max-width:360px}}
h2{{text-align:center;font-size:20px;margin-bottom:8px;color:#fff}}
.sub{{text-align:center;font-size:13px;color:#666;margin-bottom:24px}}
input{{width:100%;background:#0a0a0a;border:1px solid #2a2a2a;border-radius:8px;
      color:#eee;font-size:24px;padding:13px 14px;margin-bottom:14px;outline:none;text-align:center;letter-spacing:8px}}
input:focus{{border-color:#4CAF50}}
button{{width:100%;background:#4CAF50;color:#000;font-weight:700;font-size:15px;
       border:none;border-radius:10px;padding:13px;cursor:pointer;margin-top:4px}}
label{{display:flex;align-items:center;gap:8px;font-size:13px;color:#666;margin-bottom:14px;cursor:pointer}}
.err{{color:#f44;font-size:13px;text-align:center;margin-bottom:14px}}
.logo{{text-align:center;font-size:36px;margin-bottom:12px}}
a{{color:#4CAF50;font-size:12px;text-decoration:none;display:block;text-align:center;margin-top:14px}}
</style></head><body>
<div class="box">
  <div class="logo">🔐</div>
  <h2>Weryfikacja urządzenia</h2>
  <div class="sub">Pierwsze logowanie z tego urządzenia</div>
  {tg_info}
  {"<div class='err'>Nieprawidłowy kod. Spróbuj ponownie.</div>" if request.query_params.get("error") else ""}
  <form method="post" action="/login/otp?t={t}">
    ...
    <label>
      ...
      Zapamiętaj to urządzenie (30 dni)
    </label>
...
@app.post("/login/otp")
async def otp_submit(request: Request, t: str = "", otp: str = Form(...),
    ...
    if not entry or time.time() > entry.get("expires", 0):
        return RedirectResponse(url="/login?error=otp_expired", status_code=302)
    uid      = entry["uid"]
    next_url = entry.get("next", "/")
    if not _verify_otp(uid, otp):
        return RedirectResponse(url=f"/login/otp?t={t}&error=1", status_code=302)
    # OTP OK — usuń tymczasowy token
    _active_sessions.pop(f"otp_{t}", None)
    session_token = create_session(uid)
    resp = RedirectResponse(url=next_url, status_code=302)
    resp.set_cookie("session_token", session_token, max_age=SESSION_DURATION,
                    httponly=True, samesite="lax")
    if trust_device == "1":
        dt = _register_trusted_device(uid)
        resp.set_cookie("trusted_device", dt, max_age=_TRUSTED_DEVICE_TTL,
                        httponly=True, samesite="lax")
        log.info(f"2FA: nowe zaufane urządzenie dla {uid}")
    log.info(f"2FA: zweryfikowano {uid} → {next_url}")
    return resp


@app.post("/api/2fa/revoke_devices")
async def revoke_trusted_devices(request: Request):
    """Usuwa wszystkie zaufane urządzenia użytkownika (tylko własne lub admin)."""
    uid = get_uid_from_request(request)
    if not uid:
        return {"status": "error", "message": "Brak sesji"}
    _u = get_user_from_request(request)
    body = await request.json()
    target_uid = body.get("uid", uid)
    # Tylko admin może usuwać dla innych
    if target_uid != uid and _u.get("role") != "admin":
        return {"status": "error", "message": "Brak uprawnień"}
    devices = _load_trusted_devices()
    before = len(devices)
    devices = {k: v for k, v in devices.items() if v.get("uid") != target_uid}
    _save_trusted_devices(devices)
    removed = before - len(devices)
    log.info(f"2FA: usunięto {removed} zaufanych urządzeń dla {target_uid}")
...
async def push_subscribe(request: Request):
    ...
    for existing in _push_subscriptions:
        if existing.get("endpoint") == endpoint:
            changed = False
            # Zaktualizuj uid jeśli się zmienił
            if existing.get("_uid") != uid:
                existing["_uid"] = uid
                changed = True
            # Zaktualizuj klucze p256dh/auth jeśli się zmieniły (nowa subskrypcja, ten sam endpoint)
            new_keys_data = sub.get("keys", {})
            if new_keys_data and existing.get("keys") != new_keys_data:
                existing["keys"] = new_keys_data
                changed = True
                log.info(f"Zaktualizowano klucze subskrypcji push (ten sam endpoint): {endpoint[:60]}")
            if changed:
                _save_push_subs()
...
async def push_vapid_public():
    ...
    return JSONResponse(
        content={"public_key": pub or ""},
        headers={"Cache-Control": "no-store"},
    )


@app.post("/api/push/regenerate-vapid")
async def push_regenerate_vapid(request: Request):
    """Regeneruje klucze VAPID i czyści wszystkie subskrypcje (wymaga admina)."""
    uid = get_uid_from_request(request)
    users = load_users()
    u = users.get(uid, {})
    if u.get("role") != "admin":
    ...
    global _push_subscriptions
    # Generuj nowe klucze
    new_keys = _push.generate_vapid_keys()
    s = load_settings()
    s["vapid_private"] = new_keys["private_key"]
    s["vapid_public"]  = new_keys["public_key"]
    save_settings_file(s)
    # Wyczyść wszystkie subskrypcje
    _push_subscriptions = []
    _save_push_subs()
    log.info("Zregenerowano klucze VAPID i wyczyszczono subskrypcje")
    return {"status": "ok", "public_key": new_keys["public_key"]}
...
async def generate(
    ...
    log.info(f"Odebrano: {len(image_data)} bajtow")

    # Sprawdź duplikat przed zapisem
    _dup = _find_duplicate_image(image_data, _img_dir_g)
    if _dup:
        filename  = _dup
        file_path = os.path.join(_img_dir_g, filename)
        log.info(f"Deduplikacja: plik {filename} juz istnieje – pomijam zapis")
    else:
...
def load_users() -> dict:
    ...
    return _load_users_json(USERS_FILE)

def save_users(users: dict):
    _save_users_json(USERS_FILE, users)
...
def user_gen_history_file(uid: str) -> str:
    return str(BASE_DIR / f"gen_history_{uid}.json")
...
# ─── Historia generowań ────────────────────────────────────────────────────
_GEN_HISTORY_FILE = BASE_DIR / "gen_history.json"

def load_gen_history() -> list:
    return _load_history_json(_GEN_HISTORY_FILE)

def save_gen_history(entry: dict, uid: str = "", hist_file: str = ""):
    """Dodaje wpis do historii generowań (max 200 ostatnich). Per-user."""
    try:
        target_file = Path(hist_file) if hist_file else (Path(user_gen_history_file(uid)) if uid else _GEN_HISTORY_FILE)
        _append_history_json(target_file, entry, max_items=200)
    except Exception as e:
        log.warning(f"save_gen_history blad: {e}")

# ─── Push subscriptions ────────────────────────────────────────────────────
_push_subscriptions: list = []     # lista subskrypcji przeglądarek
_PUSH_SUBS_FILE = BASE_DIR / "push_subs.json"

def _load_push_subs():
    global _push_subscriptions
    _push_subscriptions = _load_push_subscriptions_json(_PUSH_SUBS_FILE)

def _save_push_subs():
    try:
        _save_push_subscriptions_json(_PUSH_SUBS_FILE, _push_subscriptions)
    except Exception:
        pass
...
def send_push_to_all(title: str, body: str):
    """Wysyła push notification do wszystkich użytkowników (ntfy + Web Push)."""
    # Najpierw wyślij ntfy do wszystkich użytkowników
    send_ntfy_to_all(title, body)

    # Potem Web Push jeśli dostępny
    ...
    for sub in _push_subscriptions:
        code = _push.send_push(sub, title, body, priv, pub)
        # Usuń tylko gdy subskrypcja WYGASŁA (404/410) – nie przy błędach naszego JWT (403)
        if code in (404, 410):
            dead.append(sub)
    for d in dead:
        try:
    ...
    if dead:
        _save_push_subs()
        log.info(f"Web Push: usunieto {len(dead)} wygaslych subskrypcji")
...
def _ntfy_header(value: str) -> str:
    """Usuwa znaki spoza latin-1 (emoji) z wartości nagłówka HTTP."""
    try:
...
def send_ntfy_image(topic_name, image_path, message_title="Wygenerowano grafikę",
                    ntfy_server: str = "", message: str = ""):
    _server = (ntfy_server or "https://ntfy.sh").rstrip("/")
    url = f"{_server}/{topic_name}"
    filename = os.path.basename(image_path)   # oryginalna nazwa – NIE zmieniana po kompresji

    NTFY_MAX_BYTES = 5 * 1024 * 1024  # 5 MB – bezpieczny limit dla ntfy.sh free tier

    # Kompresuj jeśli plik za duży
    send_path = image_path
    _temp_path = None
    try:
        if os.path.getsize(image_path) > NTFY_MAX_BYTES:
            log.info(f"ntfy: obraz {os.path.getsize(image_path)//1024}KB > 5MB, kompresuję...")
            img = Image.open(image_path)
            if img.mode in ("RGBA", "P", "LA"):
                img = img.convert("RGB")
            _temp_path = image_path + "_ntfy_tmp.jpg"
            quality = 80
            while quality >= 20:
                img.save(_temp_path, format="JPEG", quality=quality, optimize=True)
                if os.path.getsize(_temp_path) <= NTFY_MAX_BYTES:
                    break
                quality -= 10
            send_path = _temp_path
            # filename pozostaje oryginalna (nie _ntfy_tmp.jpg)
            log.info(f"ntfy: po kompresji {os.path.getsize(_temp_path)//1024}KB (quality={quality})")

        hdrs = {
            "Title":    _ntfy_header(message_title),
            "Filename": filename,
        }
        if message:
            hdrs["Message"] = _ntfy_header(message)

        with open(send_path, "rb") as f:
            response = requests.put(          # PUT = attachment upload (per ntfy docs)
                url,
                data=f,
                headers=hdrs,
                timeout=30
            )
        log.info(f"ntfy: status {response.status_code} dla {filename}")
...
def send_ntfy_to_user(uid: str, title: str, body: str, image_path: str = None):
    """
    Wysyła powiadomienie ntfy do konkretnego użytkownika.
    Jeśli image_path podany - wysyła obrazek jako załącznik.
    Pomijane jeśli ntfy_disabled=True w profilu użytkownika.
    """
    users = load_users()
    user = users.get(uid, {})
    if not user:
        log.warning(f"ntfy: użytkownik {uid} nie istnieje")
        return

    if user.get("ntfy_disabled"):
        log.info(f"ntfy: wyłączone dla {uid} – pomijam")
        return
    
    topic = user.get("ntfy_topic", "")
    
    log.info(f"ntfy: uid={uid}, topic={topic!r}")
    
    if not topic or topic.strip() == "":
        topic = uid
        log.warning(f"ntfy: brak tematu dla {uid}, używam loginu '{uid}' jako temat")
    
    if not topic:
        log.info(f"ntfy: użytkownik {uid} nie ma skonfigurowanego ntfy - pomijam")
        return
    
    try:
        _ntfy_srv = (user.get("ntfy_server", "") or "https://ntfy.sh").rstrip("/")
        if image_path and os.path.exists(image_path):
            # Użyj nowej funkcji send_ntfy_image do wysyłki z obrazkiem
            status = send_ntfy_image(topic, image_path, title, ntfy_server=_ntfy_srv, message=body)
            log.info(f"ntfy: wysłano do {uid} (temat: {topic}, serwer: {_ntfy_srv}) - status: {status}")
        else:
            # Tylko tekst - użyj nagłówków HTTP (poprawna metoda dla ntfy.sh)
            server = (user.get("ntfy_server", "https://ntfy.sh") or "https://ntfy.sh").rstrip("/")
            url = f"{server}/{topic}"
            resp = requests.post(url, data=body.encode("utf-8"), headers={
                "Title": _ntfy_header(title),
                "Content-Type": "text/plain",
                "Priority": "3",
            }, timeout=15)
            log.info(f"ntfy: wysłano do {uid} (temat: {topic}) - status: {resp.status_code}")
    except Exception as e:
        log.warning(f"ntfy błąd wysyłki do {uid}: {e}")

def send_ntfy_to_all(title: str, body: str):
    """Wysyła powiadomienie ntfy do wszystkich użytkowników z skonfigurowanym tematem."""
    users = load_users()
    sent_count = 0
    for uid, user in users.items():
        topic = user.get("ntfy_topic", uid)
        server = user.get("ntfy_server", "https://ntfy.sh").rstrip("/")
        if not topic:
            topic = uid
        try:
            url = f"{server}/{topic}"
            ...
            log.warning(f"ntfy błąd wysyłki do {uid}: {e}")
    log.info(f"ntfy: wysłano do {sent_count} użytkowników")

def send_push_to_user(uid: str, title: str, body: str, image_path: str = None):
    """Wysyła push notification tylko do subskrypcji danego użytkownika (Web Push + ntfy)."""
    # Najpierw spróbuj ntfy jeśli skonfigurowane (z obrazkiem jeśli podano)
    send_ntfy_to_user(uid, title, body, image_path=image_path)
    ...
    log.info(f"Web Push do {uid}: {len(matching)}/{len(_push_subscriptions)} subskrypcji pasuje. Title: {title}")
    dead = []
    for sub in _push_subscriptions:
        if sub.get("_uid") != uid:
            continue
        ...
        if ok:
            log.info(f"Web Push wysłany do {uid} OK")
        else:
            log.warning(f"Web Push błąd dla {uid}")
            dead.append(sub)
    for d in dead:
        try:
            _push_subscriptions.remove(d)
...
# ─── 2FA: Trusted Device / OTP ────────────────────────────────────────────────
# Przechowuje oczekujące OTP: {uid: {"code": "123456", "expires": timestamp}}
_pending_otp: dict = {}
# Czas ważności OTP (10 minut)
_OTP_TTL = 600
# Czas ważności trusted device cookie (30 dni)
_TRUSTED_DEVICE_TTL = 30 * 24 * 3600

def _generate_otp(uid: str) -> str:
    """Generuje 6-cyfrowy OTP i zapisuje go dla użytkownika."""
    import random as _rng
    code = f"{_rng.randint(0, 999999):06d}"
    _pending_otp[uid] = {"code": code, "expires": time.time() + _OTP_TTL}
    return code

def _verify_otp(uid: str, code: str) -> bool:
    """Sprawdza OTP. Jednorazowe — usuwa po weryfikacji."""
    entry = _pending_otp.get(uid)
    if not entry:
        return False
    if time.time() > entry["expires"]:
        del _pending_otp[uid]
        return False
...
def _get_trusted_devices_file() -> Path:
    return BASE_DIR / "trusted_devices.json"

def _load_trusted_devices() -> dict:
    """Wczytuje mapę trusted device tokenów: {token: {uid, expires}}"""
    p = _get_trusted_devices_file()
    try:
        if p.exists():
            ...
            # Usuń wygasłe
            now = time.time()
            cleaned = {k: v for k, v in data.items() if v.get("expires", 0) > now}
            if len(cleaned) != len(data):
                _save_trusted_devices(cleaned)
            return cleaned
    except Exception:
        pass
    return {}

def _save_trusted_devices(devices: dict):
    p = _get_trusted_devices_file()
    try:
...
def _is_trusted_device(request: Request, uid: str) -> bool:
    """Sprawdza czy urządzenie ma ważny trusted device token dla tego użytkownika."""
    dt = request.cookies.get("trusted_device", "")
    if not dt:
        return False
    devices = _load_trusted_devices()
    entry = devices.get(dt)
    if not entry:
        return False
...
def _register_trusted_device(uid: str) -> str:
    """Generuje i zapisuje nowy trusted device token. Zwraca token."""
    dt = secrets.token_hex(32)
    devices = _load_trusted_devices()
    devices[dt] = {"uid": uid, "expires": time.time() + _TRUSTED_DEVICE_TTL, "created": time.time()}
    _save_trusted_devices(devices)
    return dt

def _send_otp_telegram(uid: str, code: str):
    """Wysyła OTP przez Telegram do użytkownika."""
    try:
        u = get_user_by_id(uid)
        tg_token = u.get("telegram_token", "") or TELEGRAM_BOT_TOKEN
        tg_chat  = u.get("telegram_chat", "") or TELEGRAM_CHAT_ID
        if not tg_token or not tg_chat:
            log.warning(f"2FA: brak Telegram dla {uid} – nie można wysłać OTP")
            return
        import requests as _req
        _req.post(
            f"https://api.telegram.org/bot{tg_token}/sendMessage",
            json={"chat_id": tg_chat, "text":
                f"🔐 *Kod weryfikacyjny ComfyUI*\n\n"
                f"Kod: `{code}`\n\n"
                f"Ważny przez 10 minut. Jeśli to nie Ty — zmień hasło!",
                "parse_mode": "Markdown"},
            timeout=8
        )
        log.info(f"2FA: wysłano OTP na Telegram dla {uid}")
    except Exception as e:
        log.warning(f"2FA: błąd wysyłki OTP Telegram dla {uid}: {e}")

def load_settings() -> dict:
    return _load_settings_json(SETTINGS_FILE)

def save_settings_file(data: dict):
    _save_settings_json(SETTINGS_FILE, data)
...
async def restart_comfy(request: Request):
    ...
    bat_path = os.path.join(comfy_path, "run_nvidia_gpu - NEW.bat")
    def do_restart():
...
def _find_duplicate_image(raw_bytes: bytes, directory: str) -> str | None:
    """
    Sprawdza czy identyczne zdjęcie (po SHA-256 bajtów) już istnieje w katalogu.
    Zwraca nazwę pliku istniejącego duplikatu lub None.
    Porównuje tylko pliki mobile_*.jpg (pliki wejściowe od użytkownika).
    """
    incoming_hash = hashlib.sha256(raw_bytes).hexdigest()
    try:
        for fname in os.listdir(directory):
            if not (fname.startswith("mobile_") and fname.lower().endswith(".jpg")):
                continue
            fpath = os.path.join(directory, fname)
            ...
            try:
                with open(fpath, "rb") as _fh:
                    existing_hash = hashlib.sha256(_fh.read()).hexdigest()
...
async def generate_v2(request: Request, background_tasks: BackgroundTasks):
    ...
    for role in ["image_1", "image_2"]:
        ...
        if file_field and hasattr(file_field, "read"):
            ...
            if img_data:
                # Sprawdź duplikat po SHA-256 surowych bajtów
                existing = _find_duplicate_image(img_data, _user_image_dir)
                if existing:
                    form_values[f"{role}_filename"] = existing
                    log.info(f"Deduplikacja {role}: plik {existing} już istnieje – ponownie używam")
                    continue
    ...
    if app.state.last_workflow and app.state.last_workflow != workflow_id:
        log.info("Zmiana workflow, czyszcze VRAM...")
        try:
            free_data = json.dumps({"unload_models": True, "free_memory": True}).encode("utf-8")
            req_free  = urllib.request.Request(
                f"http://{COMFY_URL}/free", data=free_data,
...
def process_in_background_v2(workflow_template: dict, iterations: int,
    ...
    app.state.is_processing  = True
    app.state.total_iter     = iterations
    app.state.gen_session_id = uuid.uuid4().hex[:10]   # unikalne ID tej sesji generowania
    _registered_prompt_ids: list = []  # do czyszczenia queue_meta w finally
    _pos_prompt = _extract_positive_prompt(workflow_template)  # wyciągnij przed pruningiem
    ...
    try:
        for i in range(iterations):
            ...
            # Znajdź wszystkie sink-nody (nic ich nie konsumuje) – to prawdziwe output nody
            # (SaveImage, SaveText, PreviewImage, ShowText itp.) niezależnie od klasy
            _referenced = set()
            for _n in workflow_template.values():
                if not isinstance(_n, dict): continue
                for _v in _n.get("inputs", {}).values():
                    if isinstance(_v, list) and len(_v) == 2 and isinstance(_v[0], (str, int)):
                        _referenced.add(str(_v[0]))
            _sink_ids   = _all_node_ids - _referenced
            _start_ids  = set(str(x) for x in output_node_ids) | _sink_ids
            log.debug(f"Auto-fix BFS start: output_node_ids={output_node_ids}, sinks={_sink_ids}")
            _needed_ids     = _get_needed_nodes(workflow_template, _start_ids)
            ...
            # Rejestruj metadane dla widoku kolejki
            _user_display = get_user_by_id(uid).get("name", uid) if uid else "?"
            app.state.queue_meta[prompt_id] = {
                "uid":        uid,
            ...
            _registered_prompt_ids.append(prompt_id)
            ...
            while True:
                ...
                if prompt_id in hist_resp:
                    ...
                    # Wyciągnij faktyczny prompt – kolejność priorytetów:
                    # 1. Nowy .txt w ComfyUI output (SaveText node)
                    # 2. Historia ComfyUI (output-node podpięty do CLIPTextEncode)
                    # 3. Fallback: template (już w _pos_prompt)
                    _txt_prompt = _read_prompt_from_txt(_iter_start_time)
                    if _txt_prompt:
                        _pos_prompt = _txt_prompt
                    else:
                        _hist_prompt = _extract_prompt_from_history(workflow_template, hist_resp[prompt_id])
                        if _hist_prompt:
                            _pos_prompt = _hist_prompt
                    if _pos_prompt:
                        log.info(f"[prompt] uzyty prompt ({len(_pos_prompt)} zn.): {_pos_prompt[:120]}")
            ...
            for out_filename, out_subfolder, out_type in out_filenames:
                ...
                try:
                    ...
                    save_gallery_meta(os.path.basename(out_path), {
                        ...
                        "prompt_full": _pos_prompt,
                        "gen_session": app.state.gen_session_id,
                        "timestamp":   time.time(),
                    }, img_dir=_IMG_DIR)
                ...
                if _TG_TOKEN and _TG_CHAT:
                    _tg_caption = f"Iter {i+1}/{iterations}"
                    if _pos_prompt:
                        _tg_caption += f"\n📝 {_pos_prompt[:800]}"
                    send_telegram_photo(out_path, i + 1, tg_token=_TG_TOKEN, tg_chat=_TG_CHAT,
                                        caption=_tg_caption)
                
                # Push notification do użytkownika - ntfy + Web Push
                if uid:
                    try:
                        send_push_to_user(
                            uid,
                        ...
                        log.warning(f"push blad: {_push_err}")
        ...
        # Wyczyść metadane kolejki dla tej sesji generowania
        for _pid in _registered_prompt_ids:
            app.state.queue_meta.pop(_pid, None)
        if not app.state.should_stop:
  ...
  <div id="card-{uid}" style="padding:14px;display:grid;grid-template-columns:1fr 1fr;gap:12px">
    <div style="display:flex;flex-direction:column;gap:8px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px">
        <div><div style="font-size:10px;color:#555;margin-bottom:3px">TG Token (puste=bez zmian)</div>
          <input class="ai" data-field="telegram_token" value="" placeholder="{'Skonfigurowany' if u.get('telegram_token') else 'Brak'}" type="password" autocomplete="off"></div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px">
        <div><div style="font-size:10px;color:#555;margin-bottom:3px">🔔 ntfy Temat (domyślnie=login)</div>
          <input class="ai" data-field="ntfy_topic" value="{u.get('ntfy_topic', uid)}" placeholder="np. moj-unikalny-temat"></div>
        <div><div style="font-size:10px;color:#555;margin-bottom:3px">🌐 ntfy Serwer (puste=ntfy.sh)</div>
          <input class="ai" data-field="ntfy_server" value="{u.get('ntfy_server', '')}" placeholder="https://ntfy.sh"></div>
        <div style="display:flex;align-items:center;gap:8px;margin-top:2px">
          <input type="checkbox" class="ai" data-field="ntfy_disabled" {"checked" if u.get("ntfy_disabled") else ""}
                 style="width:14px;height:14px;flex-shrink:0">
          <span style="font-size:11px;color:#888">Wyłącz ntfy (np. iOS – brak podglądu obrazka)</span>
        </div>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px">
...
<div class="new-form">
  ...
  <input id="n-ntfy"  placeholder="ntfy temat (puste=login)">
  <input id="n-ntfys" placeholder="ntfy serwer (puste=ntfy.sh)">
...
async function addUser() {{
  ...
  var data = {{
    ...
    ntfy_topic:  document.getElementById('n-ntfy').value.trim() || null,
    ntfy_server: document.getElementById('n-ntfys').value.trim() || null,
...
async def admin_save_user(request: Request):
    ...
    # Reszta pól
    for field in ["name", "role", "image_dir", "comfy_url", "telegram_token", "telegram_chat", "ntfy_topic", "ntfy_server"]:
        if field in body and body[field] not in (None, ""):
            ...
            # ntfy_topic i ntfy_server mogą być celowo wyczyszczone (powrót do domyślnych)
    ...
    existing["ntfy_disabled"]     = bool(body.get("ntfy_disabled", False))
...
# =========================================================
# PANEL EXTENSIONS — dodatkowe endpointy dla rozszerzonego panelu
# =========================================================

_LOG_NOISE_PATTERNS = [
    # Szum startowy / connection-refused podczas bootowania ComfyUI
    "WinError 10061", "urlopen error", "Connection refused",
    "comfy_get /queue: blad",  "comfy_get /object_info",
    "ComfyUI nie gotowy", "Retry ComfyUI",
    "urllib.error", "RemoteDisconnected",
    # Typowy DEBUG z bibliotek http
    " 200 OK", " 404 Not Found", "GET /api", "POST /api",
    "DEBUG", "httpx", "uvicorn.access",
]

@app.get("/api/panel/log_tail")
async def panel_log_tail(n: int = 60, filter: str = "smart"):
    """
    Zwraca ostatnie N linii logu serwera.
    filter=smart  – pomija znany szum startowy (domyślnie)
    filter=errors – tylko WARNING/ERROR/CRITICAL
    filter=all    – wszystko bez filtrowania
    """
    try:
        ...
        def _keep(line: str) -> bool:
            ...
            if filter == "errors":
                upper = line.upper()
                return any(lvl in upper for lvl in ("WARNING", "ERROR", "CRITICAL"))
            # smart: odrzuć DEBUG + znane wzorce szumu
        ...
        # Bierzemy ostatnie 5× n linii (nadmiar), filtrujemy, zwracamy ostatnie n
        candidate = all_lines[-n * 5:]
        kept = [l.rstrip() for l in candidate if _keep(l)]
        return {"lines": kept[-n:], "filter": filter}
    except Exception as e:
        return {"lines": [f"[błąd odczytu logu: {e}]"], "filter": filter}

@app.get("/api/panel/queue")
async def panel_queue():
    """Zwraca status kolejki ComfyUI (zlecenia + szacowane obrazy)."""
    try:
        data = comfy_get("/queue", timeout=3)
        if data is None:
            return {"running": 0, "pending": 0, "available": False,
                    "running_images": 0, "pending_images": 0}
        running_items = data.get("queue_running", [])
        pending_items = data.get("queue_pending", [])

        def _count_images(items):
            total = 0
            for item in items:
                pid = item[1] if len(item) > 1 else ""
                meta = app.state.queue_meta.get(pid, {})
                total += meta.get("total_iter", 1)
            return total

        return {
            "running":        len(running_items),
            "pending":        len(pending_items),
            "running_images": _count_images(running_items),
            "pending_images": _count_images(pending_items),
            "available":      True,
        }
    except Exception:
        return {"running": 0, "pending": 0, "available": False,
...
@app.get("/api/panel/active_sessions")
async def panel_active_sessions(request: Request):
    """Zwraca listę aktywnych sesji (tylko admin)."""
    _u = get_user_from_request(request)
    if _u.get("role") != "admin":
        return {"error": "Tylko admin"}
    now = time.time()
    users = load_users()
    sessions = []
    for token, sess in list(_active_sessions.items()):
        if now > sess.get("expires", 0):
            continue
        uid = sess.get("user_id", "?")
        uname = users.get(uid, {}).get("name", uid) if uid else "?"
        remaining_min = int((sess["expires"] - now) / 60)
        sessions.append({"uid": uid, "name": uname, "expires_in_min": remaining_min})
    return {"sessions": sessions, "count": len(sessions)}

@app.get("/api/panel/recent_images")
async def panel_recent_images(limit: int = 8):
    """Zwraca ostatnie N wygenerowanych obrazów (telegram_result_*) jako base64 thumbnail.
    Skanuje wszystkie katalogi użytkowników z users.json."""
    from fastapi.responses import JSONResponse
    try:
        # Zbierz wszystkie katalogi: globalny + per-user
        dirs_to_scan = set()
        dirs_to_scan.add(IMAGE_DIR)
        try:
            for _uid, _udata in load_users().items():
                _udir = _udata.get("image_dir", "")
                if _udir and os.path.isdir(_udir):
                    dirs_to_scan.add(_udir)
        except Exception:
            pass
        ...
        for img_dir in dirs_to_scan:
            try:
                for fname in os.listdir(img_dir):
                    ...
                    all_files.append((os.path.getmtime(fpath), fpath, fname))
            except Exception:
                pass

        # Sortuj od najnowszych, weź limit
        all_files.sort(key=lambda x: x[0], reverse=True)
        result = []
        for mtime, fpath, fname in all_files[:limit]:
            try:
...
async def api_free_vram(request: Request):
    """Ręczne wyczyszczenie VRAM ComfyUI (tylko admin)."""
    _u = get_user_from_request(request)
    if _u.get("role") != "admin":
        from fastapi.responses import JSONResponse
        return JSONResponse({"status": "error", "message": "Tylko admin"}, status_code=403)
    try:
        free_data = json.dumps({"unload_models": True, "free_memory": True}).encode("utf-8")
        ...
        log.info("VRAM wyczyszczony ręcznie z panelu")
        return {"status": "ok"}
    except Exception as e:
        log.warning(f"free_vram error: {e}")
        return {"status": "error", "message": str(e)}

@app.get("/api/panel/uptime")
async def panel_uptime():
    """Zwraca czas działania serwera i licznik generacji."""
    return {
        "uptime_s": int(time.time() - _server_start_time),
        "total_generated": getattr(app.state, "total_generated_count", 0),
        "is_processing": getattr(app.state, "is_processing", False),
        "current_iter": getattr(app.state, "current_iter", 0),
        "total_iter": getattr(app.state, "total_iter", 0),
    }
